...
public class ArtifactPluginDescriptorFactory implements ArtifactDescriptorFactory<ArtifactPluginDescriptor> {
  ...
  public ArtifactPluginDescriptor create(File pluginJarFile) throws ArtifactDescriptorCreateException {
    try {
      ...
      String expectedPluginJsonFilePath = MULE_ARTIFACT_FOLDER + separator + MULE_PLUGIN_JSON;
      Optional<byte[]> jsonDescriptorContentOptional =
          loadFileContentFrom(pluginJarFile, expectedPluginJsonFilePath);
      return jsonDescriptorContentOptional
          ...
          .orElseThrow(() -> new ArtifactDescriptorCreateException(pluginDescriptorNotFound(expectedPluginJsonFilePath,
                                                                                            pluginJarFile)));
  ...
  protected static String pluginDescriptorNotFound(String expectedPluginJsonPath, File pluginFile) {
    return format("The plugin descriptor '%s' on plugin file '%s' is not present",
                  expectedPluginJsonPath,
                  pluginFile);
  }
